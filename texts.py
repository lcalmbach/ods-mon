...
- record_type: POI=period of interest, CP=comparison period. The goal is to compare the values of the POI with the CP values.
- period: compare a value the given period, which can be: last_month, year to date, last year, last  years. last 10 years etc. 
- period_value: for example if the target period is "day" (2024-09-10) and the compare period is last_year then the period_value 2023
...
- event_date: data that the measurement was taken. For comparison periods it is only given for min and max values and indicate when extreme events happened.
...
- significance: for a mean value, a difference is considered moderate (1) if the difference is larger than the standard deviation of the compare values and significant (2) if the difference is larger than 2x the standard deviation.

In your summary you should describe the deviations from the comparision intervals. When significant deviations from comparison intervals are encountered, mention the min or max value and when it occurred. Use expression period of interest and period of comparison instead of POI and CP.

""",
    "table_legend": """<table>
        <tr>
            <td>POI</td><td>Period of interest</td>
        </tr>
        <tr>
            <td>CP</td><td>Comparison Period</td>
        </tr>
        <tr>
            <td>Significance 1</td><td>Deviation of >1 standard deviations from mean of distribution</td>
        </tr>
        <tr>
            <td>Significance 2</td><td>Deviation of >2 standard deviations from mean of distribution</td>
        </tr>
    </table>
""",
    "user_prompt": """Please provide a short summary of the reported values. Describe the deviations from the comparision intervals. When significant deviations from comparison intervals are encountered, mention the min or max value and when it occurred. Data: 
    {}
    """,
...
The report is generated in a tabular format, and a summary is created using a Large Language Model (LLM). The summary is written in natural language and highlights the key insights from the report. Currently, it utilizes the OpenAI API with the [GPT-4o model](https://platform.openai.com/docs/models#gpt-4o). This functionality is still under development, and while the generated summaries are generally accurate, the model sometimes struggles to differentiate between relevant and irrelevant information.
